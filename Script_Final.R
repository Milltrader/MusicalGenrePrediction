...
# Creates a correlation matrix of numeric features, then identifies pairs with correlation above 0.8. 
# `findCorrelation` returns indices of features to remove, so we remove the corresponding columns to reduce multicollinearity.
...
# ----------------------------
# The following function and loop plot features in smaller "chunks" to avoid overcrowded plots. 
# They reshape the data to long format and use facet_wrap for multiple subplots.
...
# 8. Boxplots by Genre
# ----------------------------
# Similar chunk approach for boxplots to avoid overcrowding. 
...
# Plotting function for smaller chunks 
...
# 'PCA(...)' performs principal component analysis on scaled data.
# 'fviz_eig(pca_result)' shows variance explained by components.
# 'fviz_pca_ind(...)' plots data points colored by genre.
# 'fviz_pca_var(...)' shows how features contribute to the components.
...
# ----------------------------
# 'preProcess(..., method = c("center","scale"))' subtracts each feature's mean (centering)
# and divides by its standard deviation (scaling) to normalize all features.
...
# For formula-based models (SVM), we combine X and y in one frame:
...
# 15. XGBoost
# ----------------------------
# Convert data frames to XGBoost's internal data structure (xgb.DMatrix).
# We also convert the labels from factors to numeric (0 to num_class-1) for multi-class classification.
...
# Extract top 6 features from the XGBoost importance scores.
# Then use ggpairs to visualize pairwise relationships among those features, colored by genre.
